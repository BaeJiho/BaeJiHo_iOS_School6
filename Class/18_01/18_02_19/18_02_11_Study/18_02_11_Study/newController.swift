...
class newController: UIViewController {
    ...
    @objc func addBtnAction(_ sender:UIButton) {
        ...
        for index in count - 1..<count {
            ...
            btn = UIButton()
            btn?.frame = CGRect(x: (30 + (80 * x)), y: (margin + (120 * y)), width: 70, height: 100)
            btn?.tag = count
            btn?.setImage(UIImage.init(named: "./cards/back.png"), for: .normal)
    ...
    @objc func btnAction(_ sender:UIButton) {
        ...
        cardList1[randomIndex].frame = (btn?.bounds)!
        sender.addSubview(cardList1[randomIndex])
